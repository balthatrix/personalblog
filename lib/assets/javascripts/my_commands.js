...
class Command {
  ...
  run(commandInput) {
    ...
    if(issues.notMatched) 
      throw "tried to run command '"+this.name+"' against unmatched command '"+commandInput.parsed.name
    if(Object.keys(issues.params).length > 0) 
      throw "tried to run command '"+this.name+"' when issues present: '"+JSON.stringify(issues.params)
